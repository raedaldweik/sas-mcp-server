...
- **`render_chart` tool** — emits an interactive chart spec (type, title, data rows, x/y keys, optional stacked) for a custom UI to render with a JS charting library. The spec rides in the tool-call arguments (and is echoed in the output), so it survives an MCP host that only forwards tool calls. Does no server-side plotting; pair it with the data tools (fetch rows, then chart them).
- **Use-case scoping** — restrict an assistant to a curated subset of the environment (specific CAS tables, models, decisions) via environment variables (`USE_CASE_NAME`, `USE_CASE_DESCRIPTION`, `ALLOWED_TABLES`, `ALLOWED_MODELS`, `ALLOWED_DECISIONS`, `SCOPE_ENFORCE`). When a scope is set, list tools (`list_castables`, `list_registered_models`, `list_models_and_decisions`) return only allowed resources, a new `get_use_case` tool reports the scope to the agent, and resource-access tools refuse out-of-scope IDs (unless `SCOPE_ENFORCE=false`). With no `ALLOWED_*` set the server has full access exactly as before. Matching is forgiving (ID or name, case-insensitive; tables accept `table`, `caslib.table`, or `server.caslib.table`). Designed so a non-developer can stand up per-use-case chatbots from the SAS Retrieval Agent Manager tool-server Environment Variables tab. (`src/sas_mcp_server/usecase.py`, 14 new tests in `tests/test_usecase.py`)
- **Container entrypoint with `MCP_MODE`** — the Docker image now selects its server with `MCP_MODE` (`http-direct` default, `http`, or `stdio`) via `docker-entrypoint.sh`, so the published image is ready to be hosted directly by SAS Retrieval Agent Manager as a Container MCP Server with no custom command.
- **Direct HTTP mode** (`uv run app-http-direct`) — serves the MCP protocol over streamable HTTP while authenticating to Viya with the `.env` credentials (password grant, cached token), for server-to-server MCP clients that cannot perform browser OAuth, such as SAS Retrieval Agent Manager. Optional `MCP_API_KEY` environment variable protects the endpoint with a static key (sent as `X-API-Key` or `Authorization: Bearer`); `/health` remains open for probes. `MCP_TRANSPORT` selects streamable HTTP (`http`, default, endpoint `/mcp`) or Server-Sent Events (`sse`, endpoint `/sse`).
- **`explain_data` tool** — natural-language insights about a table column via the Insights service (`/insights/explain`)
- **New MCP tools** across these tiers:
...
### Fixed
- `create_ml_project` — Projects opened in Model Studio showed "The analytics project creation step failed to update project metadata. Make sure that the parameters passed to the automation project creation endpoint are valid." Root cause: the request sent invalid `analyticsProjectAttributes` (`targetLevel`, `classSelectionStatistic`) that broke the underlying analytics-project metadata step. Aligned the payload with SAS's documented MLPA examples — `analyticsProjectAttributes` now contains only `targetVariable`, `partitionEnabled`, and (for classification) `targetEventLevel`; SAS auto-detects the target measurement level — set `settings.applyGlobalMetadata=false`, and negotiate the SAS media type via the `Accept: application/vnd.sas.analytics.ml.pipeline.automation.project+json` header.
